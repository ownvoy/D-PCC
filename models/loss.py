...
def get_chamfer_loss(gt_xyzs, pred_xyzs, args):
    ...
    for i in range(args.layer_num):
        d1, d2, _, pred2gt_idx = chamfer_dist(
            gt_xyzs[args.layer_num - 1 - i].permute(0, 2, 1).contiguous(),
...
def get_density_loss(gt_dnums, gt_mdis, pred_unums, pred_mdis, all_pred2gt_idx, args):
    ...
    for i in range(args.layer_num):
        ...
        if i == 0:
            ...
            cur_nearest_dnum = index_points(
                cur_gt_dnum.unsqueeze(1), all_pred2gt_idx[i - 1]
            ).squeeze(1)
            cur_nearest_mdis = index_points(
                cur_gt_mdis.unsqueeze(1), all_pred2gt_idx[i - 1]
            ).squeeze(1)
        ...
        cur_density_loss = (
            cur_upsample_num_loss + args.mean_distance_coe * cur_mean_distance_loss
        )
...
def get_pts_num_loss(gt_xyzs, pred_unums, args):
    ...
    for i in range(args.layer_num):
        cur_pts_num_loss = torch.abs(
            pred_unums[i].sum() - gt_xyzs[args.layer_num - 1 - i].shape[2] * batch_size
        )
...
def get_feats_loss(gt_feats, pred_feats, pred2gt_idx, args):
    # (b, c, n)
    mes_loss = nn.MSELoss()
    ...
    # print(feats_loss)
    feats_loss = feats_loss * args.feats_coe
    # print(feats_loss)
