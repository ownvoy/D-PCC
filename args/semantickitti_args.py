...
def parse_semantickitti_args():
    ...
    # optimizer
    parser.add_argument(
        "--lr", default=1e-3, type=float, help="initial learning rate for backbone"
    )
    parser.add_argument(
        "--aux_lr", default=1e-3, type=float, help="learning rate for entropy model"
    )
    parser.add_argument(
        "--weight_decay", default=0, type=float, help="weight decay for adam optimizer"
    )
    parser.add_argument(
        "--betas", default=(0.9, 0.999), type=float, help="betas for adam optimizer"
    )
    # lr scheduler
    parser.add_argument(
        "--lr_decay_step", default=15, type=int, help="learning rate decay step size"
    )
    parser.add_argument(
        "--gamma", default=0.5, type=float, help="gamma for scheduler_steplr"
    )
    # dataset
    parser.add_argument(
        "--train_data_path",
    ...
    parser.add_argument(
        "--compress_rots", default=False, type=str2bool, help="whether compress normals"
    )
    parser.add_argument(
        "--in_fdim",
        default=11,
